...
def execute(goal, workspace_id="BRIDGE_LAB", worker_profile="RECON", inject_failure=False, inject_timeout=False, job_id=None):
    ...
    try:
        ...
        (evdir/"network-contract.json").write_text(json.dumps({"network":res["network"],"internal":True,"worker_networks":[res["network"]],"proxy_networks":["bridge",res["network"]],"squid_image":SQUID_IMAGE,"provider_host":"opencode.ai","proxy_port":3128,"policy":"PROVIDER_ONLY"},indent=2),encoding="utf-8")
        ready=False
        ...
        stderr=str(e); marks["total_ms"]=int((time.monotonic()-started)*1000); out=result("TIMEOUT",job,workspace_id,None,"",marks["total_ms"],exit_code,manifest or {"schema":"deputy.recon.snapshot.v1","file_count":0,"total_bytes":0},stderr,marks); out["timeout_reason"]="PREPARATION_TIMEOUT"; out["execution_state"]="PREPARING"; out["preparation_phase"] = state.get("preparation_phase"); out["preparation_elapsed_ms"] = marks["total_ms"]; (evdir/"result.json").write_text(json.dumps(out,indent=2),encoding="utf-8"); return out
    except Exception as e:
        stderr=str(e); marks["total_ms"]=int((time.monotonic()-started)*1000); out=result(status,job,workspace_id,None,"",marks["total_ms"],exit_code,manifest or {"schema":"deputy.recon.snapshot.v1","file_count":0,"total_bytes":0},stderr,marks); (evdir/"result.json").write_text(json.dumps(out,indent=2),encoding="utf-8"); return out
    finally:
        _cleanup(res)
