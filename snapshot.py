...
def create_snapshot(deadline=None, phase_callback=None, checkpoint=None) -> dict:
    ...
    for rel, source, size in candidates:
        ...
        files.append({"path": rel.replace("\\", "/"), "size": size, "sha256": _sha256(target), "source_state": "TRACKED_MODIFIED" if rel in state_before["tracked_modified"] else "TRACKED_CLEAN"})

    manifest = {
        "schema": "deputy.recon.snapshot.v1",
    ...
    (SNAPSHOT_ROOT / "snapshot-manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    _remaining(deadline)
    ...
    comparable = ("branch", "head", "upstream", "worktree_changes", "index_changes", "untracked", "dirty")
    if any(state_before[key] != state_after[key] for key in comparable):
        raise RuntimeError("SNAPSHOT_SOURCE_CHANGED_DURING_CAPTURE")
    audit = {"state_before": state_before, "state_after": state_after, "excluded": excluded, "untracked_classification": excluded_untracked + potential_untracked, "reparse_points": reparses, "limits": {"max_file_bytes": MAX_FILE_BYTES, "max_file_count": MAX_FILE_COUNT, "max_total_bytes": MAX_TOTAL_BYTES}, "provider_exposure": DEPUTY_SHELL_PROVIDER_EXPOSURE}
    EVIDENCE_ROOT.mkdir(parents=True, exist_ok=True)
    (EVIDENCE_ROOT / "repo-state-and-audit.json").write_text(json.dumps(audit, indent=2), encoding="utf-8")
    return {"manifest": manifest, "audit": audit, "snapshot": str(SNAPSHOT_ROOT)}
